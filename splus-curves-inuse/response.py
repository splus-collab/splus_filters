...
##################### for sky ########################
atmosph_transm = ascii.read('sky_trans.ascii')
at_wave = atmosph_transm['wave']
at_transm = atmosph_transm['transm']
at_ius = interp1d(at_wave, at_transm)
...
if not save_ascii_filters:
    ax.plot(at_wave*10., at_transm, c='c', label='atmosphere')

    ax.plot(np.array(mirror_col1) * 10., mirror_col2/100., c='k', label='mirror')
...
if save_ascii_filters:
    print('saving fig filters_response_v01.png...')
    plt.savefig('filters_response_v01.png', format='png', dpi=240)
else:
    print('saving fig filters_response_all.png...')
    plt.savefig('filters_response_all.png', format='png', dpi=240)
